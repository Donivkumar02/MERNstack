...
const signup = async (req, res, next) => {
  ...
  hashedPassword=await bcrypt.hash(password, 12);
    ...
    const error =new HttpError('could not create user,pleasetry again.',500);
    return next(error);
  }
  let createdUser;
  try{
  ...
  try {
    ...
    const error = new HttpError(
      'Sign up failed, please try again later.'+err,
      500
    );
    return next(error);
  }
...
const login = async (req, res, next) => {
  ...
  try{
    ...
    const error =new HttpError('could not log you in,please check your crediantials and try again.',500);
